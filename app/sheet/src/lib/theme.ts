...
function apply(t: Theme) { const el = globalThis.document?.documentElement; if (el) el.dataset.theme = t; }
const [theme, setThemeSignal] = createSignal<Theme>(resolve());
apply(theme());
export { theme };
export function setTheme(t: Theme) { setThemeSignal(t); try { localStorage.setItem(KEY, t); } catch { /* ignore */ } apply(t); }
