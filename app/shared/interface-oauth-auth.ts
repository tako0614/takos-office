...
export function canonicalInterfaceResourceUri(value: string): string | null {
  try {
...
export function hasValidInterfaceOAuthConfiguration(input: {
  ...
  return (
    ...
    canonicalInterfaceResourceUri(input.audience ?? "") !== null &&
...
export async function authorizeInterfaceOAuthBearer(
  ...
  const expectedAudience = canonicalInterfaceResourceUri(
