...
- 各エディタは自分の vite `base` (`/docs/` 等) と Router base を持ち、storage は `storage.object`
  service export (`OBJECT_STORAGE_API_URL` / `OBJECT_STORAGE_ACCESS_TOKEN`、`/takos-docs/` `/takos-slide/`
...
- エディタ source は `app/<editor>/src/` に置く。新エディタを足すなら vite `base` + Router base を
  ...
  publish (UI surface / file handler) と `storage.object` consume を更新する。
